...
diseases_list = {
    ...
    25: 'Hypoglycemia', 31: 'Osteoarthritis', 5: 'Arthritis', 0: '(vertigo) Paroymsal Positional Vertigo', 
...
st.markdown("### Identify Potential Health Issues and Receive Customized Advice")

# Symptom selection with multiselect
...
if st.button("Predict Disease"):
    if selected_symptoms:
        ...
        if precautions.size > 0:
            st.write("- " + "\n- ".join([str(prec) for prec in precautions[0] if not pd.isna(prec)]))
        else:
            st.write("No specific precautions are available for this disease.")
        
        # Display Medications
        st.markdown("### Medications")
        st.write(", ".join(medications) if medications.size > 0 else "No specific medications are available.")
        
        # Display Recommended Diet
        st.markdown("### Recommended Diet")
        st.write(", ".join(rec_diet) if rec_diet.size > 0 else "No specific dietary recommendations are available.")
        
        # Display Suggested Workouts
        st.markdown("### Suggested Workouts")
        st.write(workout[0] if workout.size > 0 else "No specific workout recommendations are available.")
    else:
